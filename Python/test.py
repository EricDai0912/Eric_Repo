...
def calculate_operation(in_str_index, configuration):
    ...
    by simulating a cursor and append the cursor operations into the operation_list
    """
    ...
    for position in in_str_index:
        ...
        if cursor_position == position:
            ...
            operation += "p"
    # append the operation if every charactor is found in current configuration
    if operation.count("p") == len(in_str):
        # append the current letter operation into operation_list with the configuration index split by ","
        operation_list.append(operation + "," + str(keyboard.index(configuration)))
